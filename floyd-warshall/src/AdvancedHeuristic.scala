...
abstract class Heuristic(goalMatch: Map[Position, Int], edges: Map[Position, List[Position]]) extends Comparator[Node] {
  ...
  def h(n: Node): Int = {
    ...
    Node.goals.foreach { case (goalPos, _) =>
      ...
      value = boxPath(n, matchingBox, goalPos)
      val agentPath = Astar.search(edges, n.getAgent.getPosition, matchingBox.getPosition)
      n.boxes.filter(box => box.getId != matchingBox.getId && agentPath.contains(box.getPosition))
             .foreach(_ => value += 1)
  ...
  def boxPath(n: Node, matchingBox: Box, goalPos: Position) = {
    ...
    if ( boxPath.nonEmpty ) {
      if ( n.boxes.length != 1 ) {
        n.boxes.filter(box => box.getId != matchingBox.getId && boxPath.contains(box.getPosition))
          .foreach(_ => value += 1)
      }
      value += boxPath.length - 1
    }
    value
  }
